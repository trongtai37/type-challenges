...
type GetSign<A extends string> = A extends `+${string}`
  ? '+'
  : A extends `-${string}`
  ? '-'
  : '';
type GetLabel<A extends string> = A extends `${string}%` ? '%' : '';

type PercentageParser<A extends string> =
  A extends `${GetSign<A>}${infer N}${GetLabel<A>}`
    ? [GetSign<A>, N, GetLabel<A>]
    : A;
